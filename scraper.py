...
def move_from_insurgence(move):
    ...
    movelist = ["achillesheel", "ancientroar", "corrode", "crystalrush", "darkmatter", "dracojet", "dragonify", "drakonvoice", "jetstream",
                "livewire", "lunarcannon", "medusaray", "morph", "nanorepair", "newmoon", "permafrost", "retrograde", "wildfire", "wormhole", "zombiestrike"]
    return move in movelist


def ability_from_insurgence(ability):
    abilitylist = ["absolution", "ancientpresence", "blazeboost", "castlemoat", "chlorofury", "etherealshroud", "eventhorizon", "foundry", "glitch", "heliophobia", "intoxicate", "irrelephant", "learnean", "noctem", "omnitype", "pendulum",
                   "periodicorbit", "phototroph", "prismguard", "proteanmaxima", "regurgitation", "shadowdance", "sleet", "spectraljaws", "speedswap", "supercell", "syntheticalloy", "unleafed", "vaporization", "venomous", "windforce", "winterjoy"]
...
def strip(input):
    return input.replace("(","").replace(")","").replace(" ","").replace("-","").lower()
...
def extract_pokemon(text):
    ...
    for i in card:
        ...
        if "ability" in isplit[0]:
            dex[isplit[0].strip()] = re.sub(
                '([A-Z])', r' \1', isplit[1].strip()).strip()
    regex = "(?i)Basestats==(.*?){{(.*?)Stats(.*?)(?=}})"
    card = re.search(regex, text.replace("\n", "").replace(
        "\r", "").replace(" ", "")).group().split("|")
    card.pop(0)
    for i in card:
        if i == '':
            continue
...
def extract_delta_list():
    r = requests.get(
        "https://wiki.p-insurgence.com/index.php?title=Delta_Pokémon&action=raw")
    regex = "{{rdex\|(.*?)(?=}})"
    l = []
...
def extract_mega_list():
    r = requests.get(
        "https://wiki.p-insurgence.com/index.php?title=Mega_Evolution&action=raw")
    regex = "===Unofficial===(.*)"
    data = re.search(regex, r.text.replace("", ""),
                     re.DOTALL).group().replace("\n", "")
...
def extract_mega_stats(pkm):
    abilityregex = "abilitym( |)=( |)(.*?)(\n|\|)"
    r = requests.get(url_from_id(pkm[0]))
    ability = re.findall(abilityregex, r.text)[0][2]
    statsdata = re.findall("===Mega(.*?)===(.*?)}", r.text,
                           re.DOTALL)[0][1].replace("\n", "").replace(" ", "").split("|")[1:]
    data = {'ability1': ability, 'baseStats': {},
            "name": pkm[0], "formeLetter": "M"}
    for i in statsdata:
        if i == "":
            continue
        split = i.split("=")
...
def format_pokemon(i):
    ...
    newentry['baseStats'] = {"hp": i['HP'], "atk": i['Attack'],
                             "def": i['Defense'], "spa": i['SpAtk'], "spd": i['SpDef'], "spe": i['Speed']}
    newentry['abilities'] = {0: "Pickup" if ability_from_insurgence(
        i['ability1']) else i['ability1']}
    if("ability2" in i):
        newentry['abilities'][1] = "Pickup" if ability_from_insurgence(
            i['ability2']) else i['ability2']
    if("abilityd" in i):
        newentry['abilities']['H'] = "Pickup" if ability_from_insurgence(
            i['abilityd']) else i['abilityd']
    if "height-m" in i:
...
def get_megastone_info(pkm):
    ...
    stone_counter += 1
    stone['gen'] = 6
    stone['desc'] = "If held by an {}, this item allows it to Mega Evolve in battle.".format(
        pkm[0].split("-")[0])
    return stone
        ...
        for ki, vi in v.items():
            ...
            if type(vi) is dict:
                ...
                for index, (kj, vj) in enumerate(vi.items()):
                    jsonout += kj.replace("-", "") + ": " + convert_int_if_possible(
                        vj) + ("" if index == len(vi.items())-1 else ", ")
                jsonout += "}"
...
for i in mega_pokemon:
    dex = format_pokemon(extract_mega_stats(i))
    megastonelist.append(get_megastone_info(i))
    name = strip(dex['species'])
    print(name)
    out_pokemon[name] = dex
    dex_name_map[dex['num']] = name
...
file = open("insurgence/pokedex.js",
            "w").write(convert_pokemon_js_source(out_pokemon))
file = open("insurgence/learnsets.js",
            "w").write(convert_moveset_js_source(out_moveset))
file = open("insurgence/formats-data.js",
            "w").write(convert_format_js_source(out_pkmlist))
file = open("insurgence/items.js",
            "w").write(convert_megastone_js_source(megastonelist))
